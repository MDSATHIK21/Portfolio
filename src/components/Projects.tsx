...
const projects = [
  {
    ...
    status: "Completed",
    statusBg: "bg-white/10 text-white/60",
  ...
  },
  {
    ...
    name: "ClassBot",
    status: "Ongoing",
    statusBg: "bg-white/20 text-white",
  ...
  },
  {
    ...
    statusBg: "bg-white/10 text-white",
    ...
    <section id="projects" className="py-24 relative">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div
          ...
          className="mb-20"
        >
          <h2 className="text-4xl sm:text-6xl font-extrabold text-white mb-4">Projects</h2>
          <p className="text-xl text-white/40">Selected work & ongoing builds</p>
        </motion.div>
          ...
          {projects.map((project, idx) => (
            ...
            >
              <div className="absolute -right-8 -bottom-10 pointer-events-none select-none">
                <span className="text-8xl md:text-9xl font-extrabold text-white/5">{project.num}</span>
              </div>
              <div className="relative z-10">
                ...
                <p className="text-white/50 text-lg mb-8 max-w-3xl leading-relaxed">
                  {project.description}
                </p>
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((t, i) => (
                    <span key={i} className="px-3 py-1 rounded-full border border-white/10 text-white/60 text-xs">
                      {t}
                    </span>
                  ))}
                </div>
